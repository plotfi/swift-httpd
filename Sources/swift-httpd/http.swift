...
func getMimeTypeSwift(Name : UnsafePointer<Int8>!) -> String {
    ...
    let dot = NameStr[NameStr.lastIndex(of: ".")!...]
...
func doFile(filename : UnsafePointer<Int8>!,
  ...
  do {
    let Contents = try Data(contentsOf:
...
func http_proto(socket : UnsafeMutablePointer<FILE>?,
  ...
  if request == nil || strlen(request) < strlen("GET / HTTP/1.1") {
    return send_error(status: 403, socket: socket,
  ...
  if RequestSubParts.count != 3 {
    return send_error(status: 400, socket: socket,
  ...
  if PathTrim.starts(with: "/") || PathTrim.starts(with: "..") ||
      PathTrim.starts(with: "../") || PathTrim.contains("/../") ||
      PathTrim.reversed().starts(with: "../") {
    return send_error(status: 400, socket: socket,
                      title: "Bad Request",
  ...
  if !FileManager.default.fileExists(atPath: PathTrim,
                                     isDirectory:&isDir) {
    return send_error(status: 404, socket: socket,
                      title: "File Not Found", text: "")
  }
  ...
  if isDir.boolValue {
    ...
    if FileManager.default.fileExists(atPath: PathTrim + "index.html",
                                       isDirectory:&isDir) {
      PathTrim += "index.html"
    } else {
      readDir = true
